...
void test_hook(VM* vm, i32 idx, i16 revents) {
    ...
    if idx == g_aidx {
        if (revents & NET_POLLIN) != 0 {
            u8[16] b;
            i32 n = net_try_recv(fd, &b[0], 16);
            if n > 0 { ignore net_try_send(fd, &b[0], n); }   // echo
            net_fd_close(fd);
...
i32 main() {
    // a platform without a net arm reports failure from net_init, and
    // the reactor would wait forever for events.
    if !net_init() {
