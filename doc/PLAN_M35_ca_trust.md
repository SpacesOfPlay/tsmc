...
1. **I1 — X.509 parser (A) + DN/host matchers (F).** `src/tls_x509.mc` +
   `test/unit/test_x509.mc` with a handful of **embedded** real DER certs
   (leaf/intermediate/root, ECDSA + RSA) as byte arrays: assert the parser
   extracts issuer/subject/validity/SPKI/SAN/basicConstraints correctly,
   DN-compare matches issuer↔subject across a real chain, and the hostname
   matcher accepts/rejects the right names (incl. wildcards, embedded-NUL,
   partial-wildcard rejection). Deterministic, no network.
